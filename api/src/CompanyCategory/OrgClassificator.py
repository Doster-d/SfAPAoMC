...
class OrgClassificator:
    ...
    async def classify_company(
        ...
        async for chunk in self.link.fetch_company_entities_in_chunks(
            ...
            await self.classify_companies(
                chunk, patent_type, classify_categories, target_classifier
            )
    ...
    async def classify_patent(self, patent_type="design"):
        ...
        for patent_type in ["design", "model", "invention"]:
            chunks = []
            async for chunk in self.link.fetch_holder_entities_in_chunks(
                5000000, patent_type
            ):
                chunks = list(set(chunk["company_id"].to_list() + chunks))
            chunks = [str(i) for i in chunks if type(i) != str]
            await self.classify_company(
                chunks, patent_type, False, self.global_classification[patent_type]
            )
