...
class PatentProcessor:
    ...
    def read_patents(self, read_object, use_cols=None):
        ...
        self.current_patents_dataframe["registration_date"] = (
            self.current_patents_dataframe[
                "registration_date"
            ].apply(
                lambda date: datetime.strptime(date, date_format)
                if pd.notna(date) and has_numbers(date)
                else unix_start_time
            )
        )
        self.current_patents_dataframe["actual"] = self.current_patents_dataframe[
            "actual"
        ].apply(lambda state: state if pd.notna(state) else False)
