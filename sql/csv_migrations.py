...
def process_company(
    ...
    for chunk in pd.read_csv(
        path_to_file, sep=";", chunksize=chunk_size, usecols=use_columns, dtype=dtype
    ):
        company_data = chunk.rename(
            columns={
...
def process_patent(
    ...
    dtype = {"registration number": str, "registration date": str, "actual": bool}
    date_format = "%Y%m%d"
    unix_start_time = datetime.strptime("19700101", date_format)
    patent_data = pd.read_csv(path_to_file, usecols=use_columns, dtype=dtype)
    ...
    patent_data["patent holders"] = patent_data.apply(
        lambda row: row["authors"]
        if pd.isna(row["patent holders"]) and row["authors"] != ""
        else row["patent holders"],
        axis=1,
    )
    patent_data["registration date"] = patent_data["registration date"].apply(
        lambda date: datetime.strptime(date, date_format)
        if pd.notna(date)
        else unix_start_time
    )
    patent_data["actual"] = patent_data["actual"].apply(
        lambda state: state if pd.notna(state) else False
    )
    patent_data = patent_data.rename(
        columns={
    ...
    if patent_type == "design":
        patent_data["industrial design name"] = patent_data[
            "industrial design name"
        ].apply(lambda desc: desc if pd.notna(desc) else "")
        patent_data = patent_data.rename(
            columns={"industrial design name": "description"}
        )
        ...
        patent_data = patent_data.rename(columns={"utility model name": "description"})
    else:
        patent_data["invention name"] = patent_data["invention name"].apply(
            lambda desc: desc if pd.notna(desc) else ""
        )
        patent_data = patent_data.rename(columns={"invention name": "description"})
    patent_data.to_csv(
        save_to, quotechar="'", index=False, na_rep="NULL", quoting=csv.QUOTE_ALL
    )
...
process_patent(
    path_to_file="data/PatentDatabaseFromPublicData/PMOD.csv",
    use_columns=[
        "registration number",
        "registration date",
        "authors",
    ...
    patent_type="model",
    save_to="data/ProcessedData/PMOD.csv",
)

process_patent(
    path_to_file="data/PatentDatabaseFromPublicData/inventionsOD.csv",
    use_columns=[
        "registration number",
        "registration date",
        "authors",
    ...
    save_to="data/ProcessedData/inventionsOD.csv",
)
